...
opts_chunk$set(message=FALSE, warning=FALSE, background='#F7F7F7', fig.align='center', dev='png', retina = 1.5, tidy=FALSE, verbose=FALSE)
...
tps <- tactile.theme(superpose.line=list(col = hcl.colors(12), lwd = 2))
...
cr <- colorRampPalette(hcl.colors(20))


p.1 <- segplot(texture ~ pwp + fc, data=x, 
        horizontal = TRUE, 
        level = awc, col.regions = cr,
...
p.2 <- segplot(texture_sort1 ~ pwp + fc, data=x, 
        horizontal = TRUE, level = awc, col.regions = cr,
        main='Available Water Holding Capacity\nUSDA-ARS ROSETTA Model Centroids',
