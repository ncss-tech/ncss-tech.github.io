...
### Compare Dendrograms
```{r fig.width=10, fig.height=6}
d.ordinal <- profile_compare(x, vars=c('texture_class', 'constant'), max_d=100, k=0, rescale.result=TRUE)

d.nominal <- profile_compare(x, vars=c('texture_class_nominal', 'constant'), max_d=100, k=0, rescale.result=TRUE)

p.ordinal <- as.phylo(as.hclust(diana(d.ordinal)))
p.nominal <- as.phylo(as.hclust(diana(d.nominal)))


dueling.dendrograms(p.1 = p.ordinal, p.2 = p.nominal, lab.1 = 'Ordinal', lab.2 = 'Nominal')


# dendextend::dend_diff(as.dendrogram(diana(d.ordinal)), as.dendrogram(diana(d.nominal)))
