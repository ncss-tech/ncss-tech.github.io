...
x$texture_class_nominal <- factor(x$texture_class, ordered = FALSE)

# order plasticity
x$plasticity <- factor(x$plasticity, levels=c('nonplastic', 'slightly plastic', 'moderately plastic'))

# graphical check
par(mar=c(0,1,3,1))
plot(x, color='texture_class', label='pedon_id')
plot(x, color='plasticity', label='pedon_id')
```
...
par(mar=c(2,1,3,1))
plotProfileDendrogram(x, dd.hz, scaling.factor = 0.75, y.offset = 5, width=0.2, color='texture_class', label='pedon_id', cex.name=0.65, col.palette=brewer.pal(10, 'Spectral'))
...
# compute pair-wise distances using texture class (ordered factor) and rock frag volume
d.hz <- profile_compare(x, vars=c('texture_class_nominal', 'total_frags_pct'), max_d=100, k=0, filter=idx, replace_na=TRUE, add_soil_flag=TRUE)
# divisive hierarchical clustering
dd.hz <- diana(d.hz)
