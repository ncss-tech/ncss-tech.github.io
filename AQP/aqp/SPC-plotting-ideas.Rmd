...
plotSPC(x, print.id = TRUE)
# add PSCS brackets as vertical var
addBracket(s, offset = -0.33, tick.length = 0, lwd = 10, col = rgb(red=0, green=0, blue=1, alpha=0.25))
...
addDiagnosticBracket(x, offset = -0.33, kind='paralithic contact', col='black')
```
...
# plot profiles in the new order
par(mar=c(4.5,1,1,1))
plotSPC(x, plot.order = new.order, print.id = TRUE, relative.pos = pos[new.order])
...
# move horizon designations into profiles
groupedProfilePlot(
  mineralKing, 
  groups = 'taxonname', 
  print.id = FALSE, 
...
plotTransect(mineralKing, grad.var.name='elev_field', crs=crs.utm, grad.axis.title='Elevation (m)', label='pedon_id', name='hzname', width=0.15, spacing = 'relative', fix.relative.pos = list(maxIter=6000, adj=0.2, thresh=0.7))
