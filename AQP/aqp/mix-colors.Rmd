...
Neutral spectra now available (2025-12-10).
```{r fig.height=5.5, fig.width=9}
plotColorMixture(c('5Y 8/10', 'N 4/'), label.cex = 0.65, showMixedSpec = TRUE, mixingMethod = 'exact')
```
...
Red + grey (available 2025-12-10).
```{r fig.height=2.5, fig.width=10}
s <- seq(0, 1, length.out = 6)
z <- .continuousMixture(c('5R 4/6', 'N 6/'), s)
...
# remove neutral chips
x <- subset(x, subset = hue != 'N')
