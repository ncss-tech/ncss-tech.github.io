...
## Counterintuitive Mixture Candidates
...
Check: "blue" + "yellow" = "green", yes.
```{r fig.height=5.5, fig.width=9}
plotColorMixture(c('5B 5/10', '5Y 8/8'), swatch.cex = 4, label.cex = 0.65)
```

Add weighted geometric mean spectra (dotted line): closest chip isn't a perfect fit.
```{r fig.height=5.5, fig.width=9}
...
# x <- munsell.spectra[munsell.spectra$hue == '10YR', ]
# x <- munsell.spectra[munsell.spectra$hue == '10YR' & munsell.spectra$value > 4, ]
...
xyplot(
  ...
  par.settings = tps,
  # auto.key = list(cex = 0.5, lines = TRUE, points = FALSE, columns = 6),
  type = c('l', 'g'),
  ylab = 'Reflectance',
  xlab = 'Wavelength (nm)',
