...
# for now, most commonly used hue pages, but expanded to full range in value / chroma
x <- subset(munsell, subset = hue %in% c('2.5YR') & value %in% 2:8 & chroma %in% 2:8)

x <- subset(munsell, subset = value %in% 6 & chroma %in% 6)
idx <- match(huePosition(returnHues = TRUE), x$hue)
x <- x[idx, ]
...
data(munsell)
x <- subset(munsell, subset = value %in% 2:8 & chroma %in% 2:8 & hue %in% c('10R', '2.5YR', '5YR', '7.5YR', '10YR', '2.5Y', '5Y'))

## leave out N for now
# x2 <- subset(munsell, subset = value %in% 2:6 & hue %in% c('N'))
# x <- rbind(x1, x2)
...
d$.hue1 <- factor(.m1$hue, levels = huePosition(returnHues = TRUE))
d$.hue2 <- factor(.m2$hue, levels = huePosition(returnHues = TRUE))

# only viz comparisons bewtween colors with same hue
