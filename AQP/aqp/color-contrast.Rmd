...
data(munsell)
x <- subset(munsell, subset = value %in% 2:8 & chroma %in% 1:8 & hue %in% c('10R', '2.5YR', '5YR', '7.5YR', '10YR', '2.5Y', '5Y'))
...
nd <- expand.grid(dH = 0:3, dV = 0:6, dC = 0:6)
nd$dE00 <- predict(m, newdata = nd)
...
tps <- tactile.theme(regions = list(col = viridis(200, alpha = 0.75, direction = -1)))

levelplot(
  dE00 ~ dV * dC | dH, 
  data = nd,
  main = 'Expected Change in dE00',
  xlab = expression(Delta*Value),
  ylab = expression(Delta*Chroma),
  as.table = TRUE,
  par.settings = tps,
  ...
  panel = function(x, y, z, subscripts, ...) {
    ...
    .txt[.NAidx] <- ''
    panel.text(x[subscripts], y[subscripts], labels = .txt, col = 'black', cex = 0.66, font = 2)
  }
)
