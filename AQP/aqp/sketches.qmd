...
# ugh, looks like a recent bug in quarto + pandoc + svglite
# https://github.com/quarto-dev/quarto-cli/issues/7369
# will need to wait for the next RStudio release
...
### offset.id
Vertical offset applied to profile ID, only when `id.style = 'top'`
...
## addBracket()
Add vertical brackets next to profile sketches, smart enough follow ordering adjusted with `plot.order` and `plot.order` and `relative.pos`. By default, bracket top and bottom depths are expected in columns named for horizon depths defined in `x`, from the last call to `plotSPC(x)`. Override with the `hzDepths` argument.
```{r}
#| fig.width = 10,
#| fig.height = 5
par(mar = c(0, 0, 0, 1.5))
...
# brackets defined by interval spanning "B" horizons
# note that we are using the same horizon depth column names as used by `osds`
.brackets <- data.frame(
  id = profile_id(osds),
...
# annotate "B" horizons
addBracket(.brackets, offset = - 0.45, labcol = 'label')
```
