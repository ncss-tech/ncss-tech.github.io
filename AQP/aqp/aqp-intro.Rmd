...
The following function demonstrates some of the more advanced useage of `plotSPC()`.
```{r}
explainPlotSPC <- function(x, ...) {
  ...
  # plotting order
  text(x=1:length(x), y=-10, labels=lsp$plot.order, col='darkred', font=4, cex=0.75)
  mtext('original profile index', side=3, line=-1, font=4, col='darkred')
...
lsp <- get('last_spc_plot', envir=aqp.env)
addBracket(idx=lsp$plot.order, top=a[, 1], bottom=a[, 2], col='red')
```
...
lsp <- get('last_spc_plot', envir=aqp.env)
a <- a[lsp$plot.order, ]
addBracket(idx=lsp$plot.order, top=a[, 1], bottom=a[, 2], col='red', label = site(sp4)$id, label.cex = 0.75)
```
