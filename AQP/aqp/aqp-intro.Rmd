...
# get or set site data
site(sp4)

# get or set diagnostic horizons
diagnostic_hz(sp4)

# get or set restrictions
restrictions(sp4)

# get or set spatial coordinates
coordinates(sp4)

# get or set coordinate reference system in PROJ4 notation
proj4string(sp4)
```
...
rf <- data.frame(id=pIDs, top=rf.top, bottom=rf.bottom, kind='fake', stringsAsFactors=FALSE)
...
`SoilProfileCollection` objects are combined by passing a list of objects to the `union` function. Ideally all objects share the same internal structure, profile ID, horizon ID, depth units, and other parameters of a `SoilProfileCollection`. Manually subset the example data into 3 pieces, compile into a list, and then `union` back together.
...
# create the union of all three subsets
s <- union(list(s1, s2, s3))
...
It is possible to union `SoilProfileCollection` objects with different internal structure. Columns from all inputs will be merged by `plyr::rbind.fill` into the new horizon and site level attributes. IDs and horizon depth names are taken from the first object.
```{r eval=FALSE}
...
# label each group
sp1$g <- rep('sp1', times=length(sp1))
sp3$g <- rep('sp3', times=length(sp3))

# union
x <- union(list(sp1, sp3))
...
# graphical check
plot(x, color='hzID')
groupedProfilePlot(x, 'g', color='hzID', group.name.offset = -15)
```


## Splitting
The inverse of `union` is `split`: subsets of the `SoilProfileCollection` are split into list elements, each containing a new `SoilProfileCollection`.
```{r eval=FALSE}
...
plot(sp4, name='name', color='clay', col.label='Clay Content (%)')
```
...
# plot again, this time using new colors
par(mar=c(0,0,3,0)) # tighter figure margins
plot(sp4, name='name', color='clay', col.palette=viridis::viridis(10), col.label='Clay Content (%)')
```
...
```{r fig.height=4, fig.width=8}
par(mar=c(0,0,3,0)) # tighter figure margins
plot(sp4, name='name', color='name', col.palette=RColorBrewer::brewer.pal(5, 'Set1'), col.label='Original Horizon Name')
```
...
plot(sp4, name='name', color='genhz', col.palette=RColorBrewer::brewer.pal(3, 'Spectral'), col.label='Generalized Horizon Name')
```
...
```{r dissimilarity, fig.height=6, fig.width=8}
library(cluster)
library(sharpshootR)
...
par(mar=c(1,1,5,1))
plotProfileDendrogram(sp4, d.diana, scaling.factor = 0.8, y.offset = 5, width=0.25, color='ex_Ca_to_Mg')
```
