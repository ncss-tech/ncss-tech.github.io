...
Note that we are using `names()` -- for `lm.influence()` output of the `names` attribute contains the index in terms of input dataframe. We need to do this because there can be (is) an offset in length of `lm.influence()` output and the number of rows of input due to `NA` observations. 
...
cm thick, like the slices. `slice()` can return a SPC or you can just get the sliced data (`just.the.data=TRUE`), and can be returned as a 1cm slice or an arbitrary range of slices.
