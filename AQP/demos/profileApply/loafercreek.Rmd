...
_Note that it is _not_ good practice to use the `@` (slots) to _edit_ data. 

Generally, slots are for internal use only by S4 objects in their methods, or inspection while debugging. They have special functions (like `coordinates()` and `proj4string()`) designed to access/alter the contents -- _if they are meant to be altered by the user_.
...
# the S4 method defined in the SPC class knows to only coerce data 1:1 with site
...
```{r}
profileMaxClayWhat <- function(p, what = "clay") {
    ...
    return(h[d.max.idx.first, what])
}
```

```{r}
# overwrite with identical values from new generalized function the max clay content in all profiles
loafercreek$maxclay <- profileApply(loafercreek, profileMaxClayWhat)

# calculate the DEPTH TO first horizon with max clay content in all profiles
loafercreek$maxclaydepth <- profileApply(loafercreek, profileMaxClayWhat, what="hzdept")
```
