...
subtitle: 'v0.2.2 - last updated 2019/01/30'
...
Loafercreek's siblings are the soils that geographically occupy the same landscapes and parent materials, and often are components in the same mapunits. In some mapunits sibling soils are explicitly part of a component range due to inclusion of "similar soils" to the named component, or implied by bracketing of strongly contrasting components. In many other cases, mapping associated with a particular soil name does not necessarily reflect modern series concepts for that same series name.
...
Scaled data (default) is a _list_ of colors and their weights (proportion or number of horizons with that designation). 
When you specify `k` you get aggregate color data. Aggregate data is a _data.frame_ of "representataive" colors. Munsell colors are converted to LAB color space. In LAB space, mathematical operations assessing color similarity are more feasible. 
...
But first, we will create a new grouping variable for horizons. We will make it from the _generalized_ (correlated) master designation which is loaded in `loafercreek$genhz` (from Pedon Horizon Component Layer ID in NASIS).
...
Use the code below to show the three "best" Munsell chips for each extra-generalized horizon label in the SPC. 
...
In this demo, we set `slab` structure (constant thickness) to 5 cm. This was to increase the amount of depth-wise aggregation _and_ the amount of data contributing to each 5 cm slab. Using this setup, five 1 cm thick slices will contribute to each slab _from each profile_. The slabs will be 5 cm increments from the minimum top depth to the maximum bottom depth in the SPC. Data from _all_ profiles will contribute to each slab (if non-NA).
...
That is, there are good and not so good reasons why the contributing fraction (% of profiles) for a particular slab isn't 100%. 
