...
=======
`glom()` is used to get all horizons that are _within the cambic depth range_. A custom function `has_sandy_cambic()` is defined to use `glom()` on individual profiles (`p`). A pair of regular expression patterns are used to check the `glom()`-ed horizons for stratified and/or sandy textures.

>>>>>>> e399a59a8e8e3956e03d0babc6f884257cfc627d
