...
`fetchSDA_component()` does not return diagnostic feature info by default.

We need to query SDA specifically to obtain that info. Define a function that uses `SDA_query()` to do s.

```{r message=F, warning=F}
# custom function using SDA_query() to get the diagnostic table
get_SDA_diagnostics <- function(spc) {
  target_cokeys <- profile_id(spc)
...
plotSPC(my.spc.sub, color='sandtotal_r', width=0.15)
...
has_sandy_cambic <- function(p, sandy.texture.pattern = "SR|-S$|^S$|COS$|L[^V]FS$|[^L]VFS$|LS$|LFS$") {
  ...
  if(!is.null(cambic_top) | !is.null(cambic_bot)) {
    ...
    #TODO: minor bug:
    # we add 1 because clod.hz.ids() will include the overlying horizon 
    # if z1 is a horizon boundary. the reindexing is handled *correctly* for z2
    # also, some combinations cause the SPC-SPDF horizon indexing error
    cambic_horizons <- glom(p, cambic_top + 1, cambic_bot, as.data.frame = TRUE)
