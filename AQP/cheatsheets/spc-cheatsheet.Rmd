...
Note that the length of the vector `matches.your.list` equals `nrow(site(loafercreek))`, but the number of TRUE values in `matches.your.list` (`sum(matches.your.list)`) is equal to `length(your.list)`. 

This is because all profile IDs in the list were found. If `sum(matches.your.list) < length(your.list)` some in the list were NOT found.
...
Add site data to the SPC based on common (unique) site id (for example: `id`)

```{r, eval=F, warning=F, message=F}
site(your.spc) <- merge(site(your.spc), your.site.data, by = "id")
```

Add horizon data to the SPC based on common (unique) horizon id (for example: `hzid`)
