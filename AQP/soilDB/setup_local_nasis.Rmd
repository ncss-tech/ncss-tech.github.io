...
1. Go to the Start Menu and search for 'ODBC'. See [this page](http://msdn.microsoft.com/en-us/library/ms188691.aspx) for detailed instructions on how to to find the ODBC Data Source Control Panel. Select the 64-bit ODBC application if you are using Windows 10. It should look like this.
...
# index first 10 profiles (pedons)
idx <- 1:pmin(length(x), 10)

# make sketches of select profiles
par(mar=c(0,0,0,0))
plot(x[idx, ], name = 'hzname', label = 'taxonname')
```
