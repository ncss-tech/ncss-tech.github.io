...
Required packages, be sure to ge tthe [latest versions from GitHub](https://github.com/ncss-tech/aqp).
```{r}
library(aqp)
library(soilDB)
```
...
text(1:nrow(z), 0.15, sprintf('%s/%s', z$wt.y, z$wt.x))
```
...
# Next Step: Mix via Munsell Spectra
