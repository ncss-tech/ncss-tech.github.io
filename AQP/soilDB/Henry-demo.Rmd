...
xyplot(sensor_value ~ date_time | name, data=x$soiltemp, type=c('l', 'g'), as.table=TRUE, layout=c(2,5), xlab='Date', ylab='Deg C')
xyplot(sensor_value ~ date_time | name, data=x$soilVWC, type=c('l', 'g'), as.table=TRUE, layout=c(2,5), xlab='Date', ylab='Deg C')
```
...
data=x$soiltemp, layout=c(2,5), col.regions=c('grey', 'RoyalBlue'), cuts=1, 
...
levelplot(factor(!is.na(sensor_value)) ~ doy * factor(year) | name, 
data=x$soilVWC, layout=c(1,4), col.regions=c('grey', 'RoyalBlue'), cuts=1, 
colorkey=FALSE, as.table=TRUE, scales=list(alternating=3, cex=0.75), 
par.strip.text=list(cex=0.85), strip=strip.custom(bg='yellow'), 
xlab='Julian Day', ylab='Year')
```
...
data=x$soiltemp, layout=c(2,5), col.regions=cols,
...
levelplot(sensor_value ~ doy * factor(year) | name, main='Daily Soil Temperature at 50cm\nDeg. C',
data=x$soilVWC, layout=c(1,4), col.regions=cols,
colorkey=list(space='top'), as.table=TRUE, scales=list(alternating=3, cex=0.75), 
par.strip.text=list(cex=0.85), strip=strip.custom(bg='grey'), 
xlab='Julian Day', ylab='Year')
```
