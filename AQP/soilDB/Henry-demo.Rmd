...
```{r echo=FALSE}
kable(head(d[, c('user_site_id', 'name', 'sensor_depth', 'MAST', 'Winter', 'Summer', 'STR', 'functional.yrs', 'complete.yrs', 'dslv')]), row.names = FALSE)
```
...
Same idea, this time with soil moisture data.
```{r fig.width=9, fig.height=7}
p <- levelplot(sensor_value ~ doy * factor(year) | sensor_name + factor(sensor_depth), main='Daily Soil Moisture',
data=x$soiltemp, col.regions=cols.temp,
...
Aggregate over years by sensor / Julian day, 50cm data only.
```{r fig.width=9, fig.height=6}
...
data=a, col.regions=cols.temp, xlab='Julian Day', ylab='',
...
```{r echo=FALSE}
kable(as.data.frame(x$sensors)[which(!is.na(x$sensors$MAST)), c('user_site_id', 'sensor_depth', 'name', 'MAST', 'Winter', 'Summer', 'STR', 'functional.yrs', 'complete.yrs', 'gap.index')], row.names=FALSE)
```
...
```{r echo=FALSE}
kable(head(wl.agg), row.names=FALSE)
```
