...
This is rough outline for comparing data associated with a single soil series to all of the data associated with parent *family*. For example, compare the [Coulstone](https://casoilresource.lawr.ucdavis.edu/sde/?series=Coulstone) series too all series in the *loamy-skeletal, siliceous, semiactive, mesic typic paleudults* family.

First, lets take a look at where this family sits in relationship to the parent subgroup (typic paleudults).
<iframe id="seriesTree-frame" src="https://soilmap2-1.lawr.ucdavis.edu/seriesTree/?series=Coulstone" width="100%" height="800px">
</iframe>
...
Invoke `fetchOSD` for all series that share family-level classification with Coulstone There are better ways of doing this, but the code in this document is mostly for educational purposes.
```{r fig.width=8, fig.height=6}
# get competing series OSD data
spc <- sameFamily('Coulstone')
...
Aggregate just Coulstone data and compare with entire family. Note that Coulstone records make up about 30% of the data within this family.
```{r fig.width=9, fig.height=6}
# compare a single profile to the group-level aggregate values
a <- slab(kssl.data[which(kssl.data$taxonname == 'COULSTONE'), ], ~ clay + estimated_ph_h2o + cec7 + bs82)
