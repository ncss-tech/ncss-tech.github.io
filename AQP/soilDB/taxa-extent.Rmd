...
## Formative Elements
```{r fig.height=6, fig.width=7}
x <- taxaExtent('pale', type = 'formative element', level = 'greatgroup')
a <- aggregate(x, fact = 5, fun = mean, na.rm = TRUE)
...
x <- taxaExtent('grossarenic', type = 'formative element', level = 'subgroup')
...
## Family Mineralogy Classes
```{r fig.height=6, fig.width=7}
x <- taxaExtent('glassy', type = 'mineralogy class')
a <- aggregate(x, fact = 5, fun = mean, na.rm = TRUE)

# zoomed to full extent
plot(a, axes = FALSE, col = hcl.colors(25), ext = b, mar = c(1, 1, 3, 4), main = 'Family Mineralogy Class\nIncludes "glassy"')
lines(us_states, lwd = 2)
```
