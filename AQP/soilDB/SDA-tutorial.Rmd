...
This kind of operation is much operation is much simpler using a two step process via `SDA_spatialQuery()` to get a set of intersecting map unit keys, then `SDA_query()` to return additional tabular data. Some things to keep in mind when using `SDA_spatial_query()`:
