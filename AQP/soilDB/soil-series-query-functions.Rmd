...
opts_chunk$set(message=FALSE, warning=FALSE, background='#F7F7F7', fig.align='center', dev='ragg_png', fig.retina=2, tidy=FALSE, verbose=FALSE)
...
```{r, fig.width=6, fig.height=6, fig.cap="Geographically associated soils of selected soil series."}
# create a graph object from source series names and their GAS
g <- graph_from_data_frame(s$geog_assoc_soils[, 1:2]) 
...
par(mar = c(0, 0, 0, 0))
plot(g, edge.arrow.size = 0.33, vertex.label.cex = 0.5, vertex.label.family = 'sans', vertex.label.color = 'black', vertex.frame.color = NA, vertex.shape = 'none') 
```
...
A rough approximation of MLRA membership has been computed by taking the intersection of all map unit polygons and current MLRA polygons. Membership values are based on intersecting areas weighted by component percentage divided by total soil series area. Visualize MLRA membership as a graph, vertex size is proportional to log10(area).
```{r, fig.width=6, fig.height=6, fig.cap="MLRA membership for selected soil series."}
# compute a sum of area by series and MLRA for vertex size
a1 <- aggregate(s$mlra, area_ac ~ series,  FUN = sum)
a2 <- aggregate(s$mlra, area_ac ~ mlra,  FUN = sum)
...
g <- graph_from_data_frame(s$mlra[, 1:2], vertices = a) 
...
V(g)$size <- log(V(g)$ac) * 1.5

par(mar = c(0, 0, 0, 0))
plot(g, edge.arrow.size = 0.25, vertex.label.cex = 0.5, vertex.label.family = 'sans', vertex.label.color = 'black', vertex.frame.color = NA, vertex.color = 'lightblue') 
```

Ecological sites associated with each soil series.
```{r, fig.width=6, fig.height=6, fig.cap="Ecological classification membership for selected soil series."}
# compute a sum of area by series and MLRA for vertex size
a1 <- aggregate(s$ecoclassid, area_ac ~ series,  FUN = sum)
a2 <- aggregate(s$ecoclassid, area_ac ~ ecoclassid,  FUN = sum)
...
# create a graph object from source series names and MLRA members
g <- graph_from_data_frame(s$ecoclassid[, 1:2], vertices = a) 
...
.s <- reconcileOSDGeomorph(s, 'hillpos')

res <- vizHillslopePosition(.s$geom)
print(res$fig)
```
...
plotSPC(.s$SPC, plot.order = res$order, name.style = 'center-center', cex.names = 0.75, hz.depths = TRUE, hz.depths.offset = 0.05, fixLabelCollisions = TRUE, depth.axis = FALSE)
title('Hydrologic Ordering via Hillslope Position Proportions')
```

```{r fig.width=10, fig.height=4, echo=FALSE}
.s <- reconcileOSDGeomorph(s, 'geomcomp')

res <- vizGeomorphicComponent(.s$geom)
print(res$fig)
```
...
plot(.s$SPC, plot.order = res$order, name.style = 'center-center', cex.names = 0.75, hz.depths = TRUE, hz.depths.offset = 0.05, fixLabelCollisions = TRUE, depth.axis = FALSE)
title('Hydrologic Ordering via Geomorphic Component Proportions')
```


```{r fig.width=10, fig.height=4, echo=FALSE}
.s <- reconcileOSDGeomorph(s, 'geomorphons')

res <- vizGeomorphons(.s$geom)
fig <- update(res$fig, lwd = 0)
print(fig)
```
...
plot(.s$SPC, plot.order = res$order, name.style = 'center-center', cex.names = 0.75, hz.depths = TRUE, hz.depths.offset = 0.05, fixLabelCollisions = TRUE, depth.axis = FALSE)
title('Hydrologic Ordering via Geomorphons Proportions')
```

```{r fig.width=10, fig.height=6, echo=FALSE}
par(mar=c(0,0,1,1))
SoilTaxonomyDendrogram(s$SPC, y.offset = 0.4, scaling.factor = 0.015, name.style = 'center-center', cex.names = 0.75, hz.depths = TRUE, hz.depths.offset = 0.05, fixLabelCollisions = TRUE, depth.axis = FALSE, width = 0.28)
```
...
# remove profiles that are missing geomorphic records
.s <- reconcileOSDGeomorph(s, 'hillpos')
res <- vizHillslopePosition(.s$geom)
print(res$fig)
