...
Hillslope position (1D) and geomorphic component (2D) probabilities have been estimated by series by dividing the number of components assigned the each hillslope position or geomorphic component by the total number of components. The total number of components and Shannon entropy are also included in the summary.
```{r echo=FALSE}
...
## Vizualization of 1D and 2D Surface Shape Proportions
Hillslope position.
```{r fig.width=12, fig.height=5}
...
s.sub <- s$sib.data[s$sib.data$compkind == 'Series', ]
...
d <- unique(rbind(s$sib.data, s$second.sib.data))
...
# plot network diagram, with Amador soil highlighted
par(mar=c(1,1,1,1))
plotSoilRelationGraph(m, s='Amador', spanning.tree = 'max')
```
