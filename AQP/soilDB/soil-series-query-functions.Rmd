...
Take a peek at the new extended summaries.
```{r echo=FALSE}
...
kable_styling(kable(s$climate[1:10, ], format='html', digits = 2), font_size = 9, full_width = FALSE)
```
...
## TODO: this will be abstracted into fetchOSD

# split annual from monthly climate summaries
annual.data <- s$climate[grep('ppt|pet', s$climate$climate_var, invert = TRUE), ]
monthly.data <- s$climate[grep('ppt|pet', s$climate$climate_var), ]
...
monthly.data$variable <- factor(monthly.data$variable, levels = c('pet', 'ppt'), labels=c('Potential ET', 'Precipitation'))
```

```{r fig.width=12, fig.height=5}
segplot(factor(series) ~ q05 + q95 | climate_var, centers=q50, data=annual.data, draw.bands=FALSE, segments.fun=panel.arrows, ends='both', angle=90, length=1, unit='mm', scales=list(y=list(alternating=3), x=list(relation='free')), as.table=TRUE, col='black', strip=strip.custom(bg=grey(0.85), par.strip.text=list(cex=0.7)), xlab='5th-50th-95th Percentiles', panel=function(x, y, ...) {
  panel.grid(h=FALSE, v=-1, col='grey', lty=3)
  panel.abline(h=1:length(unique(annual.data$series)), col='grey', lty=3)
  panel.segplot(x, y, ...)
})
```

```{r fig.width=12, fig.height=5}
h <- ggplot(monthly.data, aes(x = month, group=series))
