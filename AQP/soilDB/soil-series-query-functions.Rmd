...
The `SoilTaxonomyDendrogram` function from `sharpshootR` package knows how to use elements from the soil classification to build a unique layout of profile sketches.
```{r}
par(mar=c(0,0,1,1))
SoilTaxonomyDendrogram(s, scaling.factor = 0.015)
```

A closer look at the object returned by `fetchKSSL`.
```{r eval=FALSE}
str(s)
siteNames(s)
horizonNames(s)
...
Here are the narratives associated with the first profile in the collection. Regular expression was used to extract specific morphologic data. Some data may be missing or incorrect due to formatting errors, inconcsistencies, or typos. Many of the OSDs were converted from papers copies via OCR.
