...
Or, if you have a local installation of the NASIS Client.
```{r, eval = FALSE}
...
# simple figure
plot(a, axes = FALSE, col = hcl.colors(n = 50, palette = 'mako'), mar = c(3, 1, 3, 3))
...
par(mar = c(0,0,1,2))
plotProfileDendrogram(osd$SPC, clust = res$clust, scaling.factor = 0.075, width = 0.33, y.offset = 0.5, name.style = 'center-center', shrink = TRUE, max.depth = 150, depth.axis = list(cex = 0.8))
...
tr <- vizGeomorphons(osd$geomorphons, dend.type = 'triangle')
print(tr$fig)
```
...
# note that some series have multiple drainage class assigned
tab <- sort(table(osd$SPC$drainagecl))
dotplot(tab, par.setings = tactile.theme())
```



### Depth Class
Include depth to silica-cemented horizons.
```{r fig.width=14, fig.height=6}

sdc <- getSoilDepthClass(osd$SPC, p = c('Cr|R|Cd|qm'))
table(sdc$depth.class)

site(osd$SPC) <- sdc

par(mar = c(0,0,1,2))
groupedProfilePlot(osd$SPC, groups = 'depth.class', name.style = 'center-center', width = 0.33, max.depth = 150, shrink = TRUE, depth.axis = list(cex = 0.8))
```
