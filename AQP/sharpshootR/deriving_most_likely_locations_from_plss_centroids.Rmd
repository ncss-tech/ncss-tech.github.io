...
p.plss.sec <- LL2PLSS(d1$lon[idx.pt], d1$lat[idx.pt], returnlevel = 'S')
...
x <- get_elev_raster(sf::st_as_sf(p.plss.sec$geom), z = 14, src = "aws")
plot(x)
plot(p.plss.sec$geom, add=TRUE)
plot(p.plss$geom, add=TRUE)
plot(d1[idx.pt,], add=TRUE)

# extract extent
e <- extent(p.plss$geom)
...
plot(d1[idx.pt,], add=TRUE)
box()
title(main="Elev(ft): QQ-Sec area with center point & most-likely areas (boxes)")
```



```{r}
# move coords for most-likely back into the original df 'd'
...
coords$id <- d1@data$id[idx.pt]
coords$dist <- a5$distance[idx]
coords <- coords[, c(3,2,1,4)]
names(coords) <- c('id', 'lat1', 'lon1', 'dist')
d6 <- join(d, coords, by='id', type='left')
```
