...
# load data - subset the data to a could of clean sites to simplify this example!!!  
...
This function will return the XY lattitude and longitude coordinates for a given formatted PLSS code. 
...
# format PLSS codes for sections identified as protracted ('PB') blocks
idx <- which(d$type=='PB')
d$plssid[idx] <- formatPLSS(d[idx, ], type="PB")

# fetch lat/long coordinates
d2 <- PLSS2LL(d)

# set type to 'UP' when coords are not returned
idx <- which(is.na(d2$lat))
d$type[idx] <- 'UP'

# reformat PLSS for unprotracted blocks
idx <- which(d$type=='UP')
...
kml_layer.SpatialPoints(d3, title='Sites_centroids', colour='royalblue', labels=paste(d3$id, d3$flag, sep=' - '), shape="http://maps.google.com/mapfiles/kml/pal2/icon18.png")
