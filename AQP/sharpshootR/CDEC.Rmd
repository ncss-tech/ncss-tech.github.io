...
```{r}
d.soil_temp <- CDECquery(id = stn, sensor = soil.temp.sensors, interval = 'E', start = '2017-09-15', end = '2018-09-15')
d.soil_moist <- CDECquery(id = stn, sensor = soil.moist.sensors, interval = 'E', start = '2017-09-15', end = '2018-09-15')
...
Query the daily precipitation measurements spanning the 2017-2018 water year from Sonora Ranger Station ([SOR](http://cdec.water.ca.gov/cgi-progs/staMeta?station_id=SOR)).

```{r, fig.height=5, fig.width=7}
# get data, note sensor number
x <- CDECquery(id='sor', sensor=45, interval='D', start='2017-09-01', end='2019-09-01')
...
# plot
xyplot(value ~ datetime, data=x, type=c('h', 'g'), scales=list(x=list(alternating=3, format="%b\n%Y", tick.number=20)), xlab='', ylab='Daily PPT (inches)', main='Sonora Ranger Station\nDaily Total Precipitation (inches)')

xyplot(cumulative ~ datetime, data=x, type=c('s', 'g'), scales=list(x=list(alternating=3, format="%b\n%Y", tick.number=20)), xlab='', ylab='Cumulative PPT (inches)', main='Sonora Ranger Station\nCumulative Precipitation (inches)')
