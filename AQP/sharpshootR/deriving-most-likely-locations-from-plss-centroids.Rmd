---
title: "Deriving most-likely locations with PLSS description area using site characteristics"
...
d <- read.csv(file='PLSS_centroid_sample.csv', sep =',', stringsAsFactors = FALSE)
...
# convert coordinates to DF with plssid, then join back to the original data
# coords <- as.data.frame(coordinates(a[idx, ]))
# coords$plssid <- d@data$plssid[idx.pt]
# coords <- coords[, c(3,2,1)]
# names(coords) <- c('plssid', 'lat1', 'lon1')
# d6 <- join(d3, coords, by='plssid', type='left')
```
