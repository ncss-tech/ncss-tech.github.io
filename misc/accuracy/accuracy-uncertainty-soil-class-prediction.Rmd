...
This document is my attempt at addressing some of the issues related to accuracy and uncertainty that I have brought up on the phone conferences. As such, it is very much a **work in progress**.
...
```{r}
p <- c(0.1, 0.1, 0.2, 0.6)
p.H <- round(shannon.H(p, length(p)), 2)
p.CI <- round(confusion.index(p), 2)
```

For example, given probabilities of [`r p`], Shannon H would be `r p.H`.
...
For example, given probabilities of [`r p`], CI would be `r p.CI`.
