...
I would like to draw an important distinction between (what I perceive as) a false dichotomy of sampling philosophies:
   ...
   1. Traditional, free-survey methods that are ideally based on both theoretical training and years of apprenticeship in the field. This is commonly poo-pooed by some in the DSM or geostats community as wholly unreliable and "biased". It also happens to be a highly efficient (although tacit) stratification over relevant soil-forming factors, or at least out best approximation via proxy. Logistical constraints are built-into this method.
...
<pre>
Pure statistical sampling (e.g. cLHS)
...
Some follow-up thoughts on the matter after talking with Jay and re-reading the ID703 document.
   ...
   3. Multiple groups of cLHS samples: e.g. tailored to specific suites of landforms or sub-regions within the modelling domain. For example, 3 groups: 1) rugged mountains, 2) dissected hills and foothills, and 3) flood plains. This make sense conceptually: how reasonable is it to "ask for" 100 site that efficiently covers the entire distribution of covariates over all three sub-regions?
...
## Similarity: BIOCLIM
   ...
   1.	expert interpretation of cLHS points, in the office:
   ...
   2.	index of local variance: does a representative buffer around each cLHS point contain a larger total variance (on average) than similar points that have been selected as representative of larger landscape features
   3.	multivariate similarity surface analysis: after initial experimentation, the BIOCLIM approach seems more stable than MESS. Unfortunately, neither of these methods work with nominal / ordinal data. Gower's distance is one possible alternative, but computation time may be prohibitive.
   4.	comparisons of similarity surfaces generated from three approaches:
